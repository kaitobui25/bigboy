/**
 * Chuyen response Nansen thanh metric BigBoy, sau do tinh score va hard gate.
 * Cac helper score/gate khong phu thuoc Apps Script nen co the unit-test bang Node.
 */
function bbDeriveWalletMetrics_(summary90, detail180Rows, inboxRow, config, now) {
  ...
  // PnL 180D khong co summary rieng trong fixture, nen cong tu detail tung token.
  var realized180 = 0;
  var profitable = 0;
  var positiveTotal = 0;
  var largestWinner = 0;
  ...
  // largest_winner_share chi chia tren tong winner duong de do muc do phu thuoc mot lenh thang lon.
  ...
  // Score la ranking; gate moi la dieu kien toi thieu de vao VERIFIED.
...
/**
 * Wallet score 0..100 gom 6 component co clamp rieng.
 * PnL cuc lon khong duoc bu cho breadth kem hoac phu thuoc mot winner.
 */
function bbCalculateWalletScore_(metrics, config, now) {
  ...
  // Win rate 60% tro len dat full 15 diem; cao hon khong cong them.
  var winRate = bbToNumber_(metrics.win_rate_90d, 0);
  var winRateScore = bbClamp_(winRate / 0.60, 0, 1) * 15;

  // Full breadth khi so token co loi dat 2 lan minimum gate, toi thieu target=10.
  ...
  // <=25% winner concentration: full diem; >=75%: 0 diem; o giua noi tuyen tinh.
  ...
  // Activity lay tu lan discovery gan nhat, khong phai giao dich lich su xa.
...
/**
 * Hard gate la cac dieu kien BAT BUOC, khong phai he thong diem thu hai.
 * Missing metric -> WAIT_DATA, khong doi missing thanh 0 roi ket luan wallet kem.
 */
...
/** Chon wallet can dung credit theo priority_score, gioi han so wallet moi run. */
...
/**
 * Scoring pipeline cho tung wallet:
 * PnL summary 90D -> full PnL detail 180D -> derive metric -> score/gate -> upsert Sheet.
 * Loi mot wallet duoc log va co the tiep tuc wallet khac; 401/403/budget thi dung ngay.
 */
    ...
    try {
      ...
      selected.forEach(function (inboxRow) {
        ...
        try {
          ...
          // Snapshot key luu hash, khong luu API key; dung de biet metric duoc tao tu input nao.
...
/** Cap nhat queue status sau scoring ma khong xoa lich su discovery. */
